...
You only write files under `slides/<id>/`. Never modify `package.json`, `open-slide.config.ts`, or existing slides unless the workspace preflight below shows this is not an open-slide workspace yet and the user explicitly wants you to initialize one.

## Step 0 — Workspace preflight

Before creating or editing slide files, verify the current directory is an open-slide workspace:

1. Check for `package.json`.
2. Confirm `package.json` has `@open-slide/core` in `dependencies`.
3. Check for `open-slide.config.ts`.

If any of those are missing, **do not create a slide yet**. Explain that open-slide needs a workspace and dev server before localhost can work. For this user's local setup, initialize the workspace against the reviewed local fork, not npm's upstream package:

```json
{
  "name": "open-slide-workspace",
  "private": true,
  "version": "0.0.0",
  "type": "module",
  "scripts": {
  ...
  "dependencies": {
    "@open-slide/core": "file:/Users/ezequiel.rivero/personal/ezemriv_repositories/open-slide/packages/core",
    "react": "^18.3.1",
    "react-dom": "^18.3.1"
  },
  "devDependencies": {
...
Also create `open-slide.config.ts` if missing:
...
After setup, continue this workflow. The preview URL is `http://localhost:5173/s/<slide-id>` while `npm run dev` is running.
...
- That the dev server must be running for localhost to work.
- If dependencies are not installed yet: `npm install`.
- If dev isn't running: `npm run dev` from the repo root.
- Then open `http://localhost:5173/s/<id>` (or refresh the home page).
